...
We developed a systematic fine-tuning protocol for few-shot and full-regime training applicable to any 2D blood vessel dataset, here applied separately to DRIVE and VessMAP. The goal is to quantify performance gains as the number of labeled examples used for adaptation increases. For the VSUNet variants we always start from VessShape-pretrained weights and update only the model parameters using the sampled subsets.
