...
\email[Corresponding author: ]{wesleygalvao@estudante.ufscar.br}
...
\begin{table}[t]
    \caption{Pretraining hyperparameters used for the VessShape dataset.}
...
We establish a baseline by training models directly on DRIVE and VessMAP without synthetic pretraining. We denote these models U-Net18 and U-Net50, and the training protocol is the same as the fine-tuning procedure. The only difference relative to the previous protocol is the lack of VessShape-pretrained weights, which exposes the network to a noisier initial phase that is potentially more dependent on texture.
...
We conducted a quantitative analysis based on the models' performance curves as a function of the number of annotated examples, shown in Figure~\ref{f:results_charts}. We chose the Dice score as the main evaluation metric, which is widely used in medical segmentation tasks. The Table \ref{tab:combined_fewshot_dice_models} summarizes the Dice scores for zero-shot and few-shot segmentation on DRIVE and VessMAP over repeated runs.
...
\begin{table*}[t]
    \caption{Dice (mean $\pm$ std) over repeated runs for zero-shot and few-shot and segmentation on DRIVE. Zero-shot only applies to VSUNet variants.}
    ...
    \begin{tabular}{l r r r r r}
        \hline
        \textbf{Dataset} & \textbf{\#Examples} & \textbf{VSUNet18} & \textbf{VSUNet50} & \textbf{UNet18} & \textbf{UNet50} \\
        \hline
        \multirow{3}{*}{DRIVE}
            & 0  & $0.656 \,\pm\, 0.000$ & $0.367 \,\pm\, 0.000$ & --- & --- \\
            & 1  & $0.759 \,\pm\, 0.007$ & $0.757 \,\pm\, 0.013$ & $0.682 \,\pm\, 0.054$ & $0.681 \,\pm\, 0.046$ \\
            & 16 & $0.794 \,\pm\, 0.000$ & $0.799 \,\pm\, 0.001$ & $0.785 \,\pm\, 0.003$ & $0.788 \,\pm\, 0.002$ \\
        \hline
        \multirow{3}{*}{VessMAP}
            & 0  & $0.757 \,\pm\, 0.000$ & $0.614 \,\pm\, 0.000$ & --- & --- \\
            & 1  & $0.589 \,\pm\, 0.080$ & $0.569 \,\pm\, 0.063$ & $0.488 \,\pm\, 0.027$ & $0.466 \,\pm\, 0.020$ \\
            & 20 & $0.843 \,\pm\, 0.013$ & $0.837 \,\pm\, 0.020$ & $0.769 \,\pm\, 0.062$ & $0.776 \,\pm\, 0.037$ \\
        \hline
    \end{tabular}
    \endgroup
\end{table*}
...
\begin{figure*}[tbp]
    ...
    \caption{Few-shot and zero-shot Dice performance on (a) DRIVE and (b) VessMAP. Curves show the mean Dice over $R{=}5$ runs and $S{=}3$ repetitions for each sample size $n$. The shaded areas represent the inter-run variability, they visually convey the stability of each method as $n$ increases. The inset isolates the zero-shot point ($n{=}0$) for VSUNet50 on DRIVE, highlighting the early performance gap relative to subsequent few-shot increments.}
    \label{f:results_charts}
\end{figure*}

A counterintuitive phenomenon occurs in Figure \ref{f:results_charts}(b) for VessMAP, where the performance of the VSUNet models decreases when moving from zero-shot to 1-shot, before recovering and surpassing the zero-shot performance with more samples. This behavior may be due to catastrophic forgetting \cite{MCCLOSKEY1989109}, where fine-tuning on a small dataset can lead the model to forget part of the previously acquired knowledge. However, as more data is introduced, the model is able to recover and even improve its performance, suggesting that the initially learned shape bias is robust and can be reinforced with additional data.

The 1-shot decrease in accuracy has also been documented in large-scale models like CLIP \cite{Radford2021LearningTV}, and this work addresses the distinction between directly specifying a concept and inferring it from an example. In CLIP, a text prompt such as "a photo of a \{label\}" serves as an explicit rule. In our case, the shape prior acts as this explicit rule communicated to the model. In contrast, when fine-tuning is performed with a single sample, the model is forced to optimize its weights based on very limited information, that contains not only the desired shape but also instance-specific details, such as noise and texture. This leads to overfitting, as instead of learning the relevant features of the new domain, the model tends to memorize specific details of that single example, such as its particular noise and texture pattern.

However, this phenomenon is not observed in the DRIVE dataset. This suggests that the initial shape prior is not as well-aligned with the DRIVE domain. Thus, the first training sample provides new and valuable information that helps the model adjust its weights in a beneficial way, rather than causing overfitting. Similarly, the model also benefits from more examples, as demonstrated in Figure \ref{f:results_charts}(a).

This analysis is then complemented by a qualitative evaluation of the generated segmentations, illustrating the differences between the models and the impact of the shape bias, as shown in Figure~\ref{f:results_fewshots_drive}. The results collectively demonstrate the effectiveness of pre-training with VessShape in instilling a strong shape bias, resulting in generalization capability and good segmentation quality in a low-sample regime.

n the DRIVE dataset, where the region of interest highlights low-caliber vessels, performance varies sharply. In zero-shot, the VSUNet18 already delineates the main vascular structure, whereas the VSUNet50 shows significant false negatives. With just one sample, both VSUNet models adapt quickly, but the VSUNet50 now exhibits superior segmentation of the fine vessels in the highlighted region. It is also noteworthy that the U-Net variants were competent in segmenting the main structure and the region of interest with reasonable quality in the evaluated example. With 16 samples, the visual differences between all models become minimal, converging to similar results.

\begin{figure*}[tbp]
    \centering
    \includegraphics[width=\textwidth]{figures/results/results_fewshots.pdf}
    \caption{Qualitative visual comparison of segmentation performance on the DRIVE and VessMAP datasets. The figure contrasts the outputs of VSUNet variants against U-Net variants across different few-shot regimes: zero-shot, one-shot, and few-shot (16-shots for DRIVE and 20-shots for VessMAP). For the U-Net variants, no zero-shot output is available. Red squares highlight specific regions of interest to facilitate comparison: an area with low-caliber vessels in DRIVE and a vascular protuberance in VessMAP.}
    \label{f:results_fewshots_drive}
\end{figure*}

For VessMAP, which features a vascular protuberance as its highlight, the VSUNet models in zero-shot capture the general topology but not the morphological detail of the bulge. The 1-shot scenario confirms the predicted performance drop, with the VSUNets generating discontinuous results and the U-Nets, without prior knowledge, producing very low-quality segmentation maps. Even so, the VSUNet18 already shows an initial refinement in the protuberance region. With 20 samples, all models improve significantly, although the U-Net variants still exhibit minor continuity flaws compared to the more cohesive outputs of the VSUNet models.

It is interesting to note the performance of VSUNet18, which, despite being a simpler model trained with less data, shows superior zero-shot performance compared to VSUNet50. Although VSUNet50 demonstrates specific advantages in certain few-shot scenarios, VSUNet18 remains competitive across all sampling regimes, presenting a balance between efficiency and generalization capability.

Um dos resultados mais significativos é a capacidade de generalização da nossa abordagem, demonstrada pelo desempenho zero-shot dos modelos pré-treinados. A principal evidência dessa capacidade é que os modelos conseguem segmentar a forma dos vasos em domínios visualmente opostos sem qualquer treinamento específico. No VessMAP, os vasos aparecem com alta intensidade sobre um fundo escuro, enquanto no DRIVE, os vasos são escuros sobre um fundo claro, e isso fica mais evidente no modelo VSUNet18. Essa habilidade de generalização sugere que o modelo aprendeu uma representação robusta da forma dos vasos, que é aplicável independentemente das características visuais específicas do domínio.

One of the most significant results is the generalization capability of our approach, demonstrated by the zero-shot performance of the pre-trained models. The main evidence of this capability is that the models can segment the shape of vessels in visually opposite domains without any specific training. In VessMAP, the vessels appear with high intensity on a dark background, whereas in DRIVE, the vessels are dark on a light background, and this is more evident in the VSUNet18 model. This generalization ability suggests that the model has learned a robust representation of vessel shape, which is applicable regardless of the specific visual characteristics of the domain.
